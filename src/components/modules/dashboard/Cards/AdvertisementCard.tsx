...
export const AdvertisementCard = (props) => {
  const navigate = useNavigate();
  function deleteImage(id) {
    axios
      .delete(`http://localhost:3001/advertisement/${id}`)
      .then((response) => {
    ...
    <Card p={"sm"}>
      <Grid p={0}>
        ...
        <Grid.Col span={8} className={"flex flex-col"} py={0} px={"xs"}>
          <div
            className={"flex-grow"}
            onClick={() => navigate("/dashboard/advertisment/" + props.data.id)}
          >
            <div className="font-bold two-line">{props.data?.title ?? ""}</div>
            <div className="two-line">{props.data?.description ?? ""}</div>
